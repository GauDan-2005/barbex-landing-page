...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 flex p-8 px-16 text-white items-center justify-between border-b-[0.0625rem] border-white/10 transition-all duration-300 ${
        isAtTop
    ...
    >
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="fixed top-0 left-0 w-full h-screen bg-black bg-opacity-90 flex flex-col justify-center items-center text-white md:hidden z-40">
          <NavDropdown
            title="Home"
            icon="+"
            iconHover="-"
          ...
          <NavDropdown
            ...
            list={["Option 1", "Option 2", "Option 3"]}
          />
          <Search className="fill-white hover:fill-primary-color cursor-pointer transition-all duration-500 mt-4" />
          <ShoppingCart className="fill-white hover:fill-primary-color cursor-pointer transition-all duration-500 mt-4" />
          <Button title="BOOKING NOW >>" className="mt-4" />
        </div>
      )}
    </nav>
  );
